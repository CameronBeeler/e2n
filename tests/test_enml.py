...
<en-media hash="abc123" type="application/pdf"/> done</en-note>"""
    ...
    link_segments = [s for s in segments if s.kind == "evernote_link"]
    resource_segments = [s for s in segments if s.kind == "resource"]
    assert any("Before" in s.text for s in text_segments)
    assert any("after" in s.text for s in text_segments)
    assert any("done" in s.text for s in text_segments)
    assert len(link_segments) == 1
    assert link_segments[0].text == "Linked Note"
    assert len(resource_segments) == 1
    assert resource_segments[0].mime_type == "application/pdf"
...
def test_plan_enml_segments_http_link_stays_inline() -> None:
    ...
    assert "http_link" in kinds
    text_segments = [s for s in segments if s.kind == "text"]
    assert any("Visit" in s.text for s in text_segments)
    assert any("today" in s.text for s in text_segments)
...
def test_plan_enml_segments_chunks_large_text_blocks() -> None:
    segments = plan_enml_segments("<en-note>one two three four</en-note>", text_block_limit=7)
    texts = [s.text for s in segments if s.kind == "text"]
